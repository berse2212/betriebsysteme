...
int OSMP_Send(const void *buf, int count, int dest ){
	...
	if(initialised) {
		...
		struct messageBlockHeader* newMessage =	(struct messageBlockHeader*) (((char*) sharedMemory) + firstEmptyMessageOffset);
		printf("Equal %d\n", emptyMessage == newMessage);

		(*emptyMessage) = message;

		printf("new message length %d\n", newMessage->payloadLength);
		printf("new message source %d\n", newMessage->sourcePid);
		printf("new message payloadOffset %d\n", newMessage->payloadOffset);
		...
		if(messageOffset[dest].lastMessageOffset != -1) {
			struct messageBlockHeader* block = (struct messageBlockHeader*) (&messageOffset[dest].lastMessageOffset);
